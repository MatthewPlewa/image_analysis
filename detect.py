...
if __name__ == "__main__":
    ...
    path = "D:\M2i\habit\cosmicray\data4-2-15flight\images\GO022186(4-9-20152-15-27PM)/"
    

    #this gives the ability to go through all of the images 
    imgs=os.listdir(path)
    start=0
    numtodo=1000000000 #to do all of the images you will have to set this to len(imgs)
    ...
    for i in range(len(results)):
        result=results[i]
        sumdata = sumdata +result
        j=90
        numpix=0
        while j< 255:
            if result[j]>0:
                numpix=numpix+result[j]
            j=j+1
        if numpix>3:
            print imgs[i]
